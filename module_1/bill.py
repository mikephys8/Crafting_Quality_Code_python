...
def build_rating_list(name_to_rating, names_final):
    ...
    """
    whatever = []
    for key, value in name_to_rating.items():
        if key in names_final:
            whatever.append([value, key])
    whatever = sorted(whatever, key=itemgetter(0))
    return whatever[::-1]
...
def filter_by_cuisine(names_matching_price, cuisine_to_names, cuisines_list):
    ...
    for cuisine in cuisines_list:
        for rest in cuisine_to_names[cuisine]:
            restAvailable.append(rest)
    for name in names_matching_price:
        if name in restAvailable:
            whatever.append(name)
    return  whatever
...
def read_restaurants(file):
    ...
    for rest in myObjs:
        for cuisine in rest['cuisines']:
            cuisine_to_names[cuisine].append(rest['name'])


    return (name_to_rating, price_to_names, cuisine_to_names)

print(recommend(FILENAME,'$',['Thai', 'Chinese', 'Pub Food']))
